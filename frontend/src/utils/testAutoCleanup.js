...
export const testAutoCleanupScenarios = () => {
  ...
  const scenarios = [
    ...
    },
    {
      ...
      token:
        'very.long.invalid.token.that.should.be.cleaned.up.because.it.is.not.a.valid.jwt.token.format',
      shouldCleanup: true,
    },
  ];
  ...
  scenarios.forEach((scenario, index) => {
    ...
    console.log(
      `🔑 Token: ${scenario.token ? scenario.token.substring(0, 30) + '...' : 'null'}`
    );
